...
def chinese_to_pinyin(chinese_str):
    return ''.join([''.join(pinyin(ch, style=Style.NORMAL)[0]) for ch in chinese_str])
...
def generate_chinese_names(num_names):
    ...
    random_names = []
    # 转换姓氏
    surnames = [chinese_to_pinyin(surname) for surname in surnames]
    given_names = [chinese_to_pinyin(given_name) for given_name in given_names]
    # 随机生成中文姓名列表
...
def generate_chinese_jiguan(num_jiguan):
    ...
    jiguan_list = [chinese_to_pinyin(jiguan) for jiguan in jiguan_list]
...
def generate_chinese_address(num_address):
    ...
    address_list = [chinese_to_pinyin(address) for address in address_list]
...
def generate_json_fields(num_fields):
    fields = ["name", "address", "jiguan", "name1", "address1", "jiguan1", "fld1", "fld2", "fld3", "fld4", "a", "b", "c", "d"]
...
def generate_json_data(num_records, chinese_names, chinese_jiguan, chinese_address, fields):
    ...
    for _ in range(num_records):
        record = {}
        for field in fields:
            if field == "name" or field == "name1":
                record[field] = random.choice(chinese_names)
            elif field == "address" or field == "address1":
                record[field] = random.choice(chinese_address)
            elif field == "jiguan" or field == "jiguan1":
