...
public class DataGenerator {
    ...
    private static List< Document > generateJsonData( int numRecords,
        ...
        for ( int i = 0; i < numRecords; i++ ) {
            ...
            for ( String field : fields ) {
                ...
                case "name":
                case "姓名":
                    record.append( field, chineseNames
                            .get( random.nextInt( chineseNames.size() ) ) );
                    break;
                case "address":
                case "地址":
                    record.append( field, chineseAddress
                            .get( random.nextInt( chineseAddress.size() ) ) );
                    break;
                default:
                    record.append( field, chineseJiguan
    ...
    public static List< Document > generateData( int numFields,
            int numRecords ) {
        int numNames = 67;
        int numJiguan = 59;
        int numAddress = 97;
        ...
        List<Document> data = generateJsonData( numRecords, chineseNames,
                chineseJiguan, chineseAddress, fields );
        return data;
    }

    public static void main( String[] args ) {
        List< Document > bsonObjects = generateData( 10, 2000 );
        for ( Document bsonObject : bsonObjects ) {
